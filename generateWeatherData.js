...
function makeWeatherData(forecast){
    ...
    var minTemp = hourly[0].apparentTemperature
    //  var prevTemp = hourly[0].apparentTemperature
    var prevTempChange = hourly[1].apparentTemperature-hourly[0].apparentTemperature
    var lowestTemp = [0, hourly[0].apparentTemperature]
    var highestTemp = [0,hourly[0].apparentTemperature]
    var previousSummary = ""
    var dateStringArr = []
    var evenDay = true
    var tickArray = []
    ...
    for(var i = 0; i<=18; i++){
        ...
        //    tempArr.push([Utilities.formatDate(new Date(hourly[i].time*1000), "America/New_York", "hh a"), hourly[i].apparentTemperature, hourly[i].precipIntensity])
        ...
        if(hourly[i].apparentTemperature< lowestTemp[1]){
            lowestTemp = [i, hourly[i].apparentTemperature]
        }
        if(hourly[i].apparentTemperature> highestTemp[1]){
            highestTemp = [i, hourly[i].apparentTemperature]
        }
        //used to find peak and trough of temperature
        /*
          if(i>0){
          var tmpTempChange = hourly[i].apparentTemperature-hourly[i-1].apparentTemperature
          if(prevTempChange*(tmpTempChange) <0 ){
          //-*-=+, +*+=+, only is negative if switches
          tempArr[i-1][5] = hourly[i-1].apparentTemperature+"Â°F"
          tempArr[i-1][6] = hourly[i-1].apparentTemperature
          ...
          //        tempAnnotPoints = hourly[i].apparentTemperature
        ...
        */
        ...
        //    tempArr.push([new Date(hourly[i].time*1000), hourly[i].apparentTemperature, hourly[i].precipIntensity, "opacity: "+hourly[i].precipProbability+";"+precipColor, summary, tempAnnot, tempAnnotPoints, tempAnnotColor, evenDay?1:0])
        // tempArr.push([hourly[i].time*1000,new Date(hourly[i].time*1000).getHours(), hourly[i].apparentTemperature, hourly[i].precipIntensity, "opacity: "+hourly[i].precipProbability+";"+precipColor, summary, tempAnnot, tempAnnotPoints, tempAnnotColor, evenDay?1:0])
        tempArr.push([hourly[i].time*1000,toDateStr(new Date(hourly[i].time*1000)), hourly[i].apparentTemperature, hourly[i].precipProbability, hourly[i].precipIntensity, precipColor, icon, summary])
